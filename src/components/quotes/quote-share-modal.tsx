...
export function QuoteShareModal({
    ...
    const confirmSent = async () => {
        ...
        try {
            await confirmQuoteSent(quoteId, 'whatsapp')
            ...
            toast.success(`Proposta marcada como enviada para ${clientName}.`)
            setShowSentConfirmation(false)
            setOpen(false)
            router.refresh()
        } catch (error) {
                    ...
                    <div className="space-y-5 py-2">
                        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h2 className="font-bold text-emerald-950">A mensagem abriu no WhatsApp</h2>
                                    <p className="mt-1 text-sm leading-5 text-emerald-900">Voce conseguiu enviar a proposta para {clientName}?</p>
                                </div>
                            </div>
                        </div>
                        <p className="text-sm leading-6 text-muted-foreground">Marque como enviado apenas depois de tocar em enviar no WhatsApp. Isso deixa seu pipeline e os lembretes confiaveis.</p>
                        ...
                        <div className="grid gap-4 py-4">
                            <div className="flex flex-col gap-3">
                                ...
                                <a href={`mailto:?subject=${emailSubject}&body=${emailBody}`} onClick={() => handleExternalShare('email')}>
                                    <Button variant="outline" className="h-14 w-full justify-start border-zinc-200 hover:bg-zinc-50"><div className="mr-3 rounded-full bg-blue-100 p-2"><Mail className="h-4 w-4 text-blue-600" /></div><div className="flex flex-col items-start leading-none"><span>Enviar por email</span><span className="mt-1 text-xs font-normal text-muted-foreground">Abre seu cliente de email</span></div></Button>
                                </a>
