...
async def run_test():
    ...
    try:
        ...
        # -> Type the provided email into the email field, type the provided password into the password field, and click the 'Acessar Painel' button to attempt login.
        ...
        # -> Click the 'Entrar' link on the homepage to open the login page again and attempt login (natural navigation instead of direct URL).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type the provided email into the email field, type the provided password into the password field, and click the 'Acessar Painel' button to attempt login (final allowed attempt).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
