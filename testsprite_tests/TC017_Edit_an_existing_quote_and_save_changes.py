...
async def run_test():
    ...
    try:
        ...
        # -> Type the email into the email field (index 6).
        ...
        # -> Click the 'Entrar' link on the landing page to open the login page (index 1517).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Input credentials into the visible login form (email index 2159, password index 2164) and click the 'Acessar Painel' button (index 2171) to attempt login again.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
        ...
        await expect(frame.locator('text=Edit Quote').first).to_be_visible(timeout=3000)
