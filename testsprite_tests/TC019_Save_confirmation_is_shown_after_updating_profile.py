...
async def run_test():
    ...
    try:
        ...
        # -> Type the email into the email input (index 6) and proceed to fill password then submit the login form.
        ...
        # -> Click the 'Entrar' link on the homepage to open the login page (index 3545). ASSERTION: 'Entrar' link is visible on the homepage (index 3545).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Refill the email and password fields and click 'Acessar Painel' (index 4199) to attempt login again, then wait for the app to load and check for the Profile link.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
