...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields and click the 'Acessar Painel' (login) button to sign in.
        ...
        # -> Click the 'Entrar' link to open the login page so the sign-in can be retried (click element index 3746).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the email and password fields on the visible login form (indexes 4387 and 4392) and click the 'Acessar Painel' button (index 4399) to attempt signing in.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
        ...
        # -> Click 'Configurações' (index 4546) to open settings and locate the Services section so a new service can be added (service name 'Brake inspection', price '120').
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div/div[2]/nav/a[4]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the service form: enter 'Brake inspection' into the Title (index 5108), enter '120' into the Price (index 5115), then click the Add/Adicionar button (index 5167) to create the service.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div/div[3]/div[2]/div/div/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Brake inspection')
        ...
        await expect(frame.locator('text=Brake inspection').first).to_be_visible(timeout=3000)
