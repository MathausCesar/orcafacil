...
async def run_test():
    ...
    try:
        ...
        # -> Type the login email into the email field (index 6) and password into the password field (index 7), then click the 'Acessar Painel' button (index 9).
        ...
        # -> Click the 'Entrar' link (index 2084) to open the login page so the sign-in process can be retried.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type the login email into input index 2725, type the password into input index 2730, then click the 'Acessar Painel' button at index 2737 to attempt sign-in.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
