...
async def run_test():
    ...
    try:
        ...
        # -> Type the email into the email field (index 8) and the password into the password field (index 10), then submit by clicking the 'Acessar Painel' button (index 11).
        ...
        # -> Click the 'Entrar' link (index 899) on the current landing page to reach the login/app area so the support widget can be accessed.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/header/div/nav/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type the email into the email field (index 1541), type the password into the password field (index 1546), then submit by clicking 'Acessar Painel' (index 1553).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/div[2]/div/div[3]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('mathauscesar@gmail.com')
